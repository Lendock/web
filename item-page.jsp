...
<html lang="en">
    ...
    <body class="bg-filter">
        ...
        <div class="container">
            ...
            <div class="row margin-top-65">
                ...
                <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4" style="margin-top:36px;">
                        ...
                        <c:if test="${sessionScope.user != null && sessionScope.user.id != car.owner.id}">
                            <form action="/ReviewRequest" method="post">
                                ...
                                <div class="listing-price">
                                    ...
                                    <span class="per-day">günlük</span>
                                </div>
                                <table class="dates">
                                    <tr>
                                        <th class="no-padding">Götürülmə Tarixi:
                                            <input type="text" class="form-control span2 no-bottom-margin" value="" name="start-date" id="from">
                                        </th>
                                        <th class="no-padding">Qaytarılma Tarixi:
                                ...
                                <div class="listing-buttons">          
                                    <button type="submit" name="submit" class="btn listing-request-button" value="Request">Sifariş</button>
                                    <a id="${listing.id}" class="btn listing-wishlist-button" value="Wishlist"><i <c:choose>
                                                <c:when test="${listing.wished}"> class="fa fa-heart fav"</c:when>
                                                <c:otherwise>class="fa fa-heart-o fav"</c:otherwise>
                                            </c:choose> aria-hidden="true"></i> Wishlist </a>  
                                </div>
                            </form>
                        </c:if>
    ...
    <script>
        <c:choose>
            <c:when test="${!empty sessionScope.user}">
        $(".listing-wishlist-button").click(function () {
            var a = $(this).find("i");
            $.ajax({
        ...
        $(document).ready(function () {
            $.getJSON("/UnavailableDates?lid=" + "${param.id}", function (data) {
                ...
                $.getJSON("/GetReviews?lid=${param.id}", function (data) {
                    ...
                    $(".listing-review-amount").html(data.length + " Rəylər");
                });
                $("#from").datepicker({
                    dateFormat: "dd.mm.yy",
                    minDate: new Date(),
    ...
    <script>
        ...
        currencySign = "<span class='azn'>M</span>";
        ...
        function calculateDays() {
            if ($("#to").datepicker("getDate") === null || $("#from").datepicker("getDate") === null) {
                ...
                $("#days").html(diffDays);
                $("#total-price").html(currencySign + diffDays * listing_price);
            }
        }
    </script>
</body>
